...
// --- INDIVIDUAL BOT TERMINATION ---
async function terminate(reason) {
	console.log(`[TERMINATE] Bot ${botName} shutting down: ${reason}`);
	try {
		saveState();
		if (activeBotDanceTimer) clearInterval(activeBotDanceTimer);
		if (activeDancePartyTimer) clearInterval(activeDancePartyTimer);
		bot.logout();
		console.log(`[TERMINATE] ${botName} logged out.`);
	} catch (err) {
		console.error(`[TERMINATE ERROR] ${botName}:`, err.message);
	}
}
...
const COMMAND_DISPATCHER = {
	...
	'!botdel': async (sender, args, isDm) => {
        ...
        const targetBotIdx = GLOBAL_BOTS.findIndex(b => b.botName.toLowerCase() === targetName);
        
        if (targetBotIdx !== -1) {
            const targetBot = GLOBAL_BOTS[targetBotIdx];
            console.log(`[BOTDEL] Terminating live instance: ${targetBot.botName}`);
            try {
                // If we have a reference to the terminate function, call it
                if (typeof targetBot.terminate === 'function') {
                    await targetBot.terminate('Deleted by Master');
                } else {
                    targetBot.logout();
                }
            } catch (e) {
                console.error(`[BOTDEL] Termination error: ${e.message}`);
            }
            GLOBAL_BOTS.splice(targetBotIdx, 1);
            send(isDm ? sender.id : null, `🛑 Terminated running instance of ${args[1]}!`, isDm);
        }
